...
function setupLevel(level) {
  ...
  enemyShips.forEach((ship, index) => {
    ...
    const levelHpBase = level === 1 ? BASE_SHIP_HP * .5 : BASE_SHIP_HP;
...
function joltElements(x, y) {
  const radius = Math.min(620, Math.max(380, window.innerWidth * .58));
  physicsTargets.forEach((el) => {
    ...
    if (force > .08 && !isProtectedHeader(el)) addCrack(el);
    if (force > .10) applyElementDamage(el, x, y, force);
  });
}
...
function shatterTextNear(x, y, directTarget) {
  ...
  if (directText && !isProtectedHeader(directText) && handleTextHit(directText, x, y)) return;
  ...
  if (candidates[0]) handleTextHit(candidates[0].el, x, y);
}
...
function triggerFunBlast(x = pointer.x, y = pointer.y) {
  ...
  if (textTarget) shatterTextBox(textTarget);
  else shatterTextNear(x, y, target);
}
